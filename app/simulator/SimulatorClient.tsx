...
// Annual rate assumed for the "bank deposit" comparison — also fed to /api/og/sim as bankReturn.
const BANK_RATE_PCT = 5;
...
function SimulatorContent() {
  ...
  // Keep the URL's query params in sync with the current result so a shared
  // link (and its server-rendered OG metadata) reflects what's on screen.
  useEffect(() => {
    ...
    const params = new URLSearchParams({
      ...
      period,
      pct:    result.returnPct.toFixed(1),
      value:  String(Math.round(result.currentValue)),
    });
    router.replace(`${pathname}?${params.toString()}`, { scroll: false });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [result, cache, amount, period]);
  ...
  const bankReturn = useMemo(
    () => amount * Math.pow(1 + BANK_RATE_PCT / 100, activeMeta.years),
